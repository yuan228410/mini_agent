"""记忆更新器：根据压缩摘要更新两层记忆（episode / memory）。"""
import time
from ..logger import logger
from ._utils import extract_tag as _extract
from .store import MemoryStore

COMPACT_PROMPT = """根据对话轮次摘要，更新长期记忆：
...
class MemoryUpdater:
    ...
    def update(self, chat_fn, round_summaries: list[str], ctx=None) -> None:
        ...
        logger.debug(f"[记忆更新] 解析结果: episode={'有' if episode else '无'}, memory={'有' if new_memory else '无'}")
