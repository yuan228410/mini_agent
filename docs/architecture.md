...
5. **输出回复** — 无工具调用且无活跃队友时，将 LLM 回复渲染输出到终端/Web
6. **实时持久化** — 通过 `persist_fn` 回调，每条消息（用户/助手/工具调用/工具结果）生成时立即写入 HistoryDB，确保消息不丢失
7. **压缩检查** — 检查当前 prompt token 是否超过阈值，若超过则触发记忆压缩
