...
基于 OpenAI / Anthropic Chat API 的智能对话 Agent，支持工具调用、技能系统、三层记忆压缩、子代理派遣、Team 协作、流式输出、多模型切换、MCP 协议、计划模式、Web 界面（多用户 + 多会话并行 + 工作空间管理 + 在线配置 + MCP管理）。
...
**状态栏：** 每轮对话结束后右对齐显示 `⚙ 模型 │ 上下文用量% │ ↑输入 ↓输出 token │ 系统提示词大小 │ 消息数`，上下文用量超过 70% 变黄、85% 变红。Web 端状态栏实时更新。Token 统计优先使用 API 返回值，API 未返回时按内容长度自动估算。
...
timeouts:
  ...
  llm_retries: 3              # LLM 请求失败重试次数（含 HTTP 429/5xx）
  llm_retry_delay: 2          # 重试间隔（秒，递增延迟）
...
通过 `mini-ai --web` 启动 Web 对话界面（默认 `http://localhost:8765`），支持流式输出、思维链/工具调用展示、亮暗主题切换、多用户隔离、多会话并行、工作空间管理、在线配置面板、模型增删、MCP服务器管理、会话持久化。Editorial 杂志编辑风设计，Markdown + 代码高亮渲染。
