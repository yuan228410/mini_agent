...
基于 OpenAI / Anthropic Chat API 的智能对话 Agent，支持工具调用、技能系统、记忆压缩、子代理派遣、Team 协作、流式输出、多模型切换、Web 界面（多用户 + 会话持久化）。
...
通过 `mini-ai --web` 启动 Web 对话界面（默认 `http://localhost:8765`），支持流式输出、思维链/工具调用展示、亮暗主题切换、多用户隔离、会话持久化。Editorial 杂志编辑风设计，Markdown + 代码高亮渲染。
